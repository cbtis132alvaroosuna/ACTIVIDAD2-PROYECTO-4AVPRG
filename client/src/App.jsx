...
function App() {
  ...
  // Función de login ORIGINAL (sin cambios)
  const handleLogin = () => {
    if (username.trim()) {
      setIsLoggedIn(true);
  ...
  const getSafeUserName = (user) => {
    ...
    if (typeof user.id === 'string') return `Usuario ${user.id.slice(0, 5)}`;
    return 'Usuario';
  };
  ...
  useEffect(() => {
    ...
    const handleNotification = (notification) => {
      setNotifications((prev) => [...prev, {
        from: typeof notification.from === 'string' ? notification.from : 'Sistema',
        message: typeof notification.message === 'string' ? notification.message : 'Notificación'
      }]);
      setTimeout(() => {
  ...
  const renderMessageContent = (msg) => {
    const senderInfo = msg.fromName 
      ? `${msg.fromName} (${msg.fromPhone || 'Sin número'})`
  ...
  // Pantalla de login (solo diseño mejorado)
  if (!isLoggedIn) {
  return (
    <div className="login-container">
      <div className="login-box">
        ...
        <h1>AG Chat</h1>
        <p className="welcome-text">Conéctate con tus amigos y familiares en AG Chat</p>
        ...
        <div className="input-group">
          ...
          <input
            ...
            // El value y onChange se mantienen para que sea editable
            value={username}
        ...
        <div className="input-group">
          ...
          <div className="phone-input-container">
            <select 
              ...
              defaultValue="+52" // Valor por defecto pero no se guarda
              // No hay onChange para que no haga nada al cambiar
            >
            ...
            <input
              ...
              // No hay value ni onChange para que no guarde nada
              // Pero sigue siendo editable por el usuario
            />
          </div>
        </div>
        ...
        <button 
          onClick={() => {
            // Solo valida el nombre (como en tu versión original)
            if (username.trim()) {
              setIsLoggedIn(true);
        ...
        <p className="terms-text">
          Al iniciar aceptas nuestros <a href="#" onClick={(e) => e.preventDefault()}>Términos</a> y <a href="#" onClick={(e) => e.preventDefault()}>Política</a>
        </p>
      </div>
    </div>
  );
